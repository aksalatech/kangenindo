...
class Gallery_model extends CI_Model
{
	...
	public function get_image_gallery_by_id($req){
		$query = $this->db->query("SELECT * FROM msimage_gallery WHERE id_gallery=".$this->db->escape($req));
		return $query->result();
	}

	public function deletedep($req){
		$this->db->where("id_gallery",$req);
		$this->db->delete("msimage_gallery");
	}

	public function adddep($data){
		$this->db->insert("msimage_gallery",$data);
	}

	public function updatedep($req,$data){
		$this->db->set($data);
		$this->db->where("id_gallery",$req);
		$this->db->update("msimage_gallery");
	}
