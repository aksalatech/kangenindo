...
class Promotion_model extends CI_Model
{
	...
	public function get_type_by_id($req){
		$query = $this->db->query("SELECT * FROM msbanner_promo WHERE id_promo=".$this->db->escape($req));
		return $query->result();
	}
	...
	public function get_all_type(){
		$query = $this->db->query("SELECT * FROM msbanner_promo");
		return $query->result();
	}
	...
	public function add($data){
		$this->db->insert("msbanner_promo",$data);
	}

	public function update($req,$data){
		$this->db->set($data);
		$this->db->where("id_promo",$req);
		$this->db->update("msbanner_promo");
	}

	public function delete_detail($req){
		$this->db->where("id_promo",$req);
		$this->db->delete("msbanner_promo");
	}
